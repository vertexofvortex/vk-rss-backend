...
class VKUsertokensMethods:
    ...
    def get_token_by_id(self, db: Session, usertoken_id: int):
        return (
            db.query(VKUsertokenModel)
            .filter(VKUsertokenModel.id == usertoken_id)
            .first()
        )
    ...
    def create_token(self, db: Session, usertoken: VKUsertokenCreate):
        ...
        add_token = insert(VKUsertokenModel).values(
            {
                "name": usertoken.name,
                "token": enc_token,
            }
    ...
    def delete_token(self, db: Session, usertoken_id: int):
        del_token = delete(VKUsertokenModel).where(VKUsertokenModel.id == usertoken_id)
        del_groups = delete(VKGroupModel).where(VKGroupModel.token_id == usertoken_id)
